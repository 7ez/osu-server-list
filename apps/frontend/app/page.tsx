  ...
  return (
    <div>
      <nav className="flex items-center w-full h-16">
        <h1 className="ml-7 text-sm md:text-xl lg:text-4xl font-extrabold lg:w-86">
          osu! server list
        </h1>
        ...
        {isLoading ? (
          <>
            {Array.from({ length: 67 }, (_, i) => (
              <Skeleton
                key={i}
                className="w-[calc(100vw-3rem)] lg:w-86 h-[114px] rounded-xl"
              />
            ))}
          </>
        ) : (
