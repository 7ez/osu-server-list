...
export function LoginEditServer(props: { server: Server, handleLogin: (key: string) => boolean }) {
  ...
  return (
    <DialogContent>
      <DialogHeader>
        ...
        <DialogDescription>
          Please enter the admin key to access admin features for <span className="font-bold">{props.server.name}</span>.
        </DialogDescription>
      </DialogHeader>
      <form onSubmit={handleSubmit} className="grid gap-4 py-2">
        <div className="grid gap-2">
          <Input ref={adminKeyInput} name="adminKey" placeholder="Enter server admin key" type="password" />
        </div>
...
export function ServerAdminEditServer(props: { adminKey: string, server: Server, updateServer: (server: Server) => void, isOpen: boolean, handleOpen: (open: boolean) => void }) {
  ...
  return (
    <DialogContent>
      ...
      <div className="flex flex-row gap-2">
        ...
        <Button variant="outline" onClick={() => props.handleOpen(false)} className="w-[50%] hover:cursor-pointer">Cancel</Button>
      </div>
    </DialogContent>
  );
}

export function ServerAdminModal(props: { server: Server, updateServer: (server: Server) => void }) {
  ...
  const handleLogin = (key: string): boolean => {
    fetch(`/api/v1/admin/${props.server.id}`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "x-admin-key": key
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger>
        <Button variant="ghost" size="sm" className="ml-2 hover:cursor-pointer">
          <FontAwesomeIcon width={8} height={8} icon={faEdit} />
        </Button>
      </DialogTrigger>
      {isAuthenticated ? (
        <ServerAdminEditServer adminKey={adminKey} updateServer={props.updateServer} server={props.server} isOpen={isOpen} handleOpen={setIsOpen} />
      ) : (
        <LoginEditServer server={props.server} handleLogin={handleLogin} />
      )}
    </Dialog>
  );
}
